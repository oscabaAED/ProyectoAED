...
vis_dat(Nombres) + theme(axis.text.x = element_text(angle = 90)) +
  labs(
    subtitle = info_registros
  )
```

Desde un primer momento podemos observar como esta manera de introducir los datos ha dado lugar a una gran cantidad de NAs introducidos por coerción en las observaciones de las frecuencias, de modo que esta estructuración de los datos seguramente no sea la opción más eficiente. Además también tenemos multitud de columnas con información de dos variables (frecuencia de nacimientos, ya sea absoluta o por 1000, y año), lo cual dificultará el acceso y manejo de los datos en etapas posteriores.  

Estos problemas, causados la forma en que hemos combinado las diferentes observaciones, se pueden resolver transformando este dataframe resultante a formato tidy, facilitando así su manipulación y análisis gráfico.  
...
```
  ...
  Podemos comprobar que al tranformar el dataframe hemos eliminado los registros que no contienen información sobre las frecuencias (que no son de utilidad para el análisis a realizar) sin reducir el número de observaciones. La estructura tidy será por tanto no solo más sencilla de manipular sino que también almacena la información de manera más eficiente.
...
Por estos factores podemos suponer que contamos con una gran cantidad de nombres de baja frecuencia, pero la cantidad de nacimientos se encuentra concentrada, mayoritariamente, en un comparativamente pequeño conjunto de nombres.
