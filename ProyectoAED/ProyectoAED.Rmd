---
title: Frecuencias de nombres de la provincia de Valencia a lo largo de las decadas de 1930 a 2020
author:
...
Para gestionar la información dispersa en múltiples archivos, es necesario leer cada fichero .csv individualmente. De sus nombres de archivo se extrae información clave, como la década y el sexo al que corresponden los registros.  

Originalmente, cada archivo cuenta con tres variables:  
...
Esta representación permite observar de manera sencilla los tipos de las variables. Podemos comprobar que estos han sido estructurados de manera correcta ya que contamos con `Nombres` de tipo `character` (propio de cadenas de texto), `Sexo` como variable de tipo factor (adecuado para variables categóricas con limitadas opciones) y todas las frecuencias presentes son de tipo numérico.  
...
Ambos problemas, podrán resolverse transformando este dataframe resultante a formato tidy, facilitando así su manipulación y análisis gráfico. 


## Convertir el dataframe a formato tidy

Dado que el dataframe Nombres contiene dos columnas separadas por cada década, se procede a su reestructuración utilizando funciones de tidyr y dplyr. Esta conversión a formato largo permite condensar estas múltiples columnas, quedando variables más claras: Nombre, Sexo, Década, Nac. y Nac./1000.  

Separar las variables de década y frecuencias en dos columnas también nos permitirá limpiar el dataframe, eliminando correspondientes a las décadas en que un nombre determinado tiene por frecuencia no disponible (`NA`) sin afectar a los registros del resto de variables.  
...
Una vez tranformado el dataframe a formato tidy, realizamos de nuevo una inspección visual de su contenido.  
...
Podemos comprobar que al tranformar el dataframe hemos eliminado los registros que no contienen información sobre las frecuencias (que no son de utilidad para el análisis a realizar) sin reducir el número de observaciones. La estructura tidy será en consecuencia más sencilla de manipular y más eficiente para contener la información disponible.  

Cabe destacar también que la nueva variable generada para las décadas es de tipo caracter, dado que para nuestro análisis las decas serán utilizadas como clasificador (variable categórica) esta estructura es adecuada, al no estar haciendo operaciones con fechas tampoco será necesario pasar a formato fecha.

## Descripción general de los datos

Previamente a realizar el estudio deseado para este conjunto de datos resultará necesario conocer de manera general, sus características.  
```{r echo=FALSE}
...
Por estos factores podemos suponer que contamos con una gran cantidad de nombres de baja frecuencia, pero la cantidad de nacimientos se encuentra concentrada, mayoritariamente, en un comparativamente pequeño conjunto de nombres.  La distribución de frecuencias es, por tanto, muy diferente gaussiana, por lo que no es conveniente usar las técnicas usuales que estén adaptadas a esta distribución.


# Objetivos (por poner algo de momento)
- Explorar la evolución temporal de la popularidad de nombres.
- Comparar tendencias por género.
- Analizar aparición y desaparición de nombres.
- Estudiar la influencia de factores sociculturales.
...
ggplot(Nombres_evol_plot, aes(x = Decada, y = `Nac./1000`, color = Nombre, group = Nombre)) +
...
if(knitr::is_latex_output()){
  ...
  kable(list(cada_sexo_nb[1:(nrow(cada_sexo_nb)/2),],  
           matrix(numeric(), nrow=0, ncol=1),
           cada_sexo_nb[ceiling(nrow(cada_sexo_nb)/2+1):nrow(cada_sexo_nb),]),
...
```
## Heatmap 25 nombres más populares a lo largo de las décadas
```{r, echo=FALSE}
