...
Nom_populares_mujeres <- Nombres_tidy %>%
  filter(Sexo == 'Mujer') %>%
  group_by(Decada) %>%
  arrange(desc(Nac.)) %>% #ordena descendentemente (cantidad de mujeres nacidas con ese nombre)
  slice(1:5) %>%
  ungroup()
print(Nom_populares_mujeres)

#Hago lo mismo para los hombres
Nom_populares_hombres <- Nombres_tidy %>%
  filter(Sexo == 'Hombre') %>%
  group_by(Decada) %>%
  arrange(desc(Nac.)) %>% 
  slice(1:5) %>%
  ungroup()
print(Nom_populares_hombres)
```
