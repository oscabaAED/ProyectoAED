...
librerias <- data.frame(
  Uso = c(
    "Importar el resto de librerías",
    "Manejo de datos",
    "Visualización",
    "Tablas"
  ),
  Librerías = c(
    "pacman",
    "dplyr, tidyr",
    "ggplot2, visdat, viridis, patchwork",
    "kable, kableExtra"
  )
)
...
if (knitr::is_latex_output()) {
  librerias |>
    knitr::kable(
      caption = "Librerías utilizadas en el análisis.",
      format  = "latex",
      booktabs = TRUE
    ) |>
...
Estas columnas se renombran sistemáticamente añadiendo la década como sufijo para diferenciarlas. Finalmente, todos los dataframes generados se unen en uno solo (`Nombres`) utilizando la columna `Nombre` como clave común. 
...
Esta representación permite observar de manera sencilla los tipos de las variables. Podemos comprobar que estos han sido estructurados de manera correcta ya que contamos con `Nombre` de tipo `character` (propio de cadenas de texto), `Sexo` como variable de tipo `factor` (adecuado para variables categóricas con limitadas opciones) y todas las frecuencias presentes son de tipo `numeric`.  
...
Cabe destacar también que la nueva variable generada para las décadas es de tipo carácter, dado que para nuestro análisis serán utilizadas como clasificador (variable categórica) esta estructura es adecúa al uso de la variable, no es necesario pasar a formato fecha ya que no se va a operar con ellas.
...
### Evolución de tendencias en los nombres
...
- Volumen de nacimientos: Representa la evolución del número total de nacimientos por década y sexo entre 1930 y 2010 _(figura 5A)_. Este gráfico permite identificar variaciones demográficas significativas, como el incremento sostenido de la natalidad hasta los años setenta (coincidente con el baby boom) y su posterior descenso durante las décadas siguientes.

- Diversidad de nombres: Muestra el número de nombres distintos registrados en cada década _(figura 5B)_. Se observa un crecimiento progresivo hasta 1970, seguido de una ligera estabilización. Este aumento refleja una tendencia hacia una mayor heterogeneidad en la elección de nombres.

- Características de los nombres: Analiza cambios estilísticos, como el porcentaje de nombres compuestos _(figura 6A)_ y la longitud media de los nombres _(figura 6B)_ respecto al total. Ambas gráficas presentan un incremento inicial hasta mediados del siglo XX y una reducción continua en las décadas posteriores. Estos resultados evidencian una transición cultural hacia nombres más breves y simples, en detrimento de los nombres tradicionales y compuestos.

- Renovación: Estudia la aparición _(figura 7A)_ y la desaparición _(figura 7B)_ de nombres por década. El número de nuevos nombres alcanza máximos locales en 1960 y en los años 2000, reflejando periodos de mayor creatividad en la elección de nombres. Tras estos momentos, en las siguientes décadas aumenta la cantidad de nombres que caen en desuso, con picos en los años 1970 y 2010, lo que marca etapas de cambio generacional en las preferencias. 
