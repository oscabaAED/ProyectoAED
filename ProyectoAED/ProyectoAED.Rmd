...
# La idea sería poder hacer un bucle que los lea todos con esto en algún momento
ficheros <- paste0('../data/',list.files('../data'))
decadas <- gsub("[.]csv","",str_extract(list.files('../data'),'[a-z0-9]+[.]csv'))
#decadas <- gsub('[3-9]0','19',decadas

# Primer dataframe al que añadiremos el resto por columnas
prueba <- read.csv(ficheros[1], header = F, sep='\t', skip=7, encoding = 'windows-1252', dec=',')
prueba[,'V3'] <- as.numeric(gsub('[.]','',prueba[,'V3']))

prueba2 <- prueba[,c(2,3,4)]
colnames(prueba2) <- c(c('Nombre'),paste0(variables[c(2,3)],'_',decadas[1]))
...
prueba2 <- prueba2[!(rowSums(is.na(prueba2))>0),]
# Ahora que lo pienso creo que para unirlo bien habría que hacer por un lado hombres y por otro mujeres, y luego todo
if(grepl('H',ficheros[1])){
  prueba2$Sexo <- 'Hombre'
} else if(grepl('M',ficheros[1])){
  prueba2$Sexo <- 'Mujer'
}

# Primer dataframe al que añadiremos el resto por columnas
prueba3 <- read.csv(ficheros[2], header = F, sep='\t', skip=7, encoding = 'windows-1252', dec=',')
prueba3[,'V3'] <- as.numeric(gsub('[.]','',prueba3[,'V3']))

prueba3 <- prueba3[,c(2,3,4)]
colnames(prueba3) <- c(c('Nombre'),paste0(variables[c(2,3)],'_',decadas[2]))
# Ahora que lo pienso creo que para unirlo bien habría que hacer por un lado hombres y por otro mujeres, y luego todo
prueba3 <- prueba3[!(rowSums(is.na(prueba2))>0),]
if(grepl('H',ficheros[1])){
  TotalH <- merge(x = prueba2, y = prueba3, by = "Nombre")
} else if(grepl('M',ficheros[1])){
  prueba2$Sexo <- 'Mujer'
}
...
while(i<=length(ficheros)){
  auxdf <- read.csv(ficheros[i], header = F, sep='\t', skip=7, encoding = 'windows-1252', dec=',')
  ...
  if(grepl('H',ficheros[i])){ # Identifica qué datos son de hombres porque hay H en el nombre del fichero, para mujeres M
    if(PrimerH){
      # Para el primero añadimos la columna de sexo y lo ponemos como base para el total
      auxdf$Sexo <- 'Hombre'
      TotalH <- auxdf
      PrimerH <- FALSE
    } else{
...
# También lo podemos guardar como un nuevo archivo si es más fácil leer eso directamente para otros pasos
# pero habrá que hacer algo para que la variable ficheros no lea ese también o lo irá duplicando cuando abramos el programa
...
# Convertimos a longer para poder obtener por separado ambas variables de nacimientos
Nombres_tidy <- Nombres_longer %>% 
  pivot_wider(names_from = "Tipo", values_from = "Valor")

# Habría que hacer un group_by decadas para poder sacar el orden de los nombres a lo largo del tiempo
```
