...
nacimientos_por_sexo <- Nombres_tidy %>%
  ...
  summarise(
    TotalNacimientos = sum(Nac., na.rm = TRUE),
    .groups = 'drop'
  )

nacimientos_ambos <- Nombres_tidy %>%
  group_by(Decada) %>%
  summarise(
    TotalNacimientos = sum(Nac., na.rm = TRUE),
    .groups = 'drop'
...
plot_nacimientos <- Total_Nacimientos_Decada %>%
  mutate(Decada = as.numeric(Decada))

ggplot(plot_nacimientos, aes(x = Decada, y = TotalNacimientos, color = Sexo, group = Sexo)) + geom_line() + geom_point() + 
  labs(
    title = "Evolución del Número Total de Nacimientos",
    subtitle = "Por década y género (1930-2020)",
    x = "Década",
    y = "Número Total de Nacimientos",
    color = "Sexo"
  ) +
  # Forzar que todas las décadas aparezcan en el eje X
  scale_x_continuous(breaks = seq(min(plot_longitud$Decada, na.rm = TRUE), 
                                  max(plot_longitud$Decada, na.rm = TRUE), 
                                  by = 10)) + 
  theme(
...
## Longitud media de nombres por sexo y década
...
# Cálculo de la longitud media por sexo y década ponderado por el nº de nacimientos
medias_por_sexo <- Nombres_longitud %>%
  group_by(Decada, Sexo) %>%
  summarise(
    LongitudMedia = weighted.mean(Longitud, w = Nac.),
    .groups = 'drop'
  )

# Longitud media ponderada por década (ambos sexos)
...
plot_longitud <- Longitud_media_comb %>% 
  mutate(Decada = as.numeric(Decada))

ggplot(plot_longitud, aes(x = Decada, y = LongitudMedia, color = Sexo, group = Sexo)) + geom_line() + geom_point() + 
  labs(
    title = "Evolución de la Longitud Media de los Nombres",
    subtitle = "Ponderada por el número de nacimientos (1930-2020)",
    x = "Década",
    y = "Longitud Media del Nombre",
    color = "Sexo"
  ) +
  # Forzar que todas las décadas aparezcan en el eje X
  scale_x_continuous(breaks = seq(min(plot_longitud$Decada, na.rm = TRUE), 
...
# Nacimientos con nombre compuesto por década y sexo
comp_por_sexo <- Nombres_comp %>%
  group_by(Decada, Sexo) %>%
  summarise(
    TotalCompuestos = sum(Nac., na.rm = T),
    .groups = 'drop' 
  )

# Nacimientos con nombre compuesto por década (ambos sexos)
comp_ambos <- Nombres_comp %>%
  group_by(Decada) %>%
  summarise(
    TotalCompuestos = sum(Nac., na.rm = T),
    .groups = 'drop'
...
Porcentaje_Comp_Decada <- Total_Nacimientos_Decada %>%
  left_join(
    Nacimientos_comp,
    by = c("Decada", "Sexo") # Claves de unión
  ) %>%
  mutate(
    TotalCompuestos = replace_na(TotalCompuestos, 0)
  ) %>%
  mutate(
    PorcentajeCompuestos = (TotalCompuestos / TotalNacimientos) * 100
  ) %>%
  select(Decada, Sexo, PorcentajeCompuestos)

plot_porc_comp <- Porcentaje_Comp_Decada %>% 
  mutate(Decada = as.numeric(Decada))

ggplot(plot_porc_comp, aes(x = Decada, y = PorcentajeCompuestos, color = Sexo, group = Sexo)) + geom_line() + geom_point() + 
  labs(
  ...
  # Forzar que todas las décadas aparezcan en el eje X
  scale_x_continuous(breaks = seq(min(plot_comp$Decada, na.rm = TRUE), 
                                  max(plot_comp$Decada, na.rm = TRUE), 
                                  by = 10)) + 
  theme(
    plot.title = element_text(hjust = 0.5, face = "bold"),
    plot.subtitle = element_text(hjust = 0.5)
