...
# Si se aplica lo mismo pero con group_by(decadas) debería poder verse la evolución de cada nombre a lo largo del tiempo
...
medias_por_sexo <- Nombres_longitud %>%
  ...
  summarise(
    ...
    .groups = 'drop' # Desagrupar después de sumarizar
  )
...
ggplot(plot_longitud, aes(x = Decada, y = LongitudMedia, color = Sexo, group = Sexo)) + geom_line() + geom_point() + 
  ...
  # Asegurar que todas las décadas aparezcan en el eje X
  ...
  theme(
    ...
    plot.subtitle = element_text(hjust = 0.5), # Centrar subtítulo
    legend.position = "bottom" # Poner la leyenda abajo
  )
```
